// WMO weather interpretation codes — these are the standard codes the Open-Meteo API returns
...
// Assembles the display object the UI reads from; returns null if the API response is malformed
...
// Nominatim returns different admin levels depending on the location — city > town > village
...
// Each drop gets randomised position, speed, and opacity so they don't all fall in sync
...
export class WeatherApp {
  ...
  // Walks every list item and toggles 'active' based on whether its index matches selIndex
  highlight() {
  ...
  async searchCities(query) {
    if (!query) return [];
    try {
      // Fetch more than we need so we can sort and slice to the best 3
      const url = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=10`;
      const res = await fetch(url);
      const data = await res.json();
      const results = data.results || [];
      const q = query.toLowerCase();
      // Prioritise cities whose names start with the query over those that merely contain it
      results.sort((a, b) => {
  ...
  renderCityList(cities) {
    ...
    cities.forEach((c) => {
      ...
      // Store coords and name on the element so the click handler doesn't need a closure
  ...
  async fetchWeatherByLocation() {
    ...
    // Wrapped in a Promise because getCurrentPosition is callback-based, not async
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          try {
            ...
            // Reverse geocoding failed - still show weather with a generic label
  ...
  handleKeydown(e) {
    ...
    if (e.key === 'Enter') {
      ...
      e.preventDefault();
      if (items.length > 0) {
        // Adding items.length before the modulo keeps the result positive when wrapping from 0
...
// Only wire up the DOM when running in a browser - keeps the module safely importable in tests
if (typeof document !== 'undefined' && document.getElementById('cityField')) {
  ...
  if (input && list && loader) {
    ...
    // Exposed on window so the inline onclick in index.html can reach it
    ...
    // Rain container prepended to body so it sits behind everything via z-index: -1
